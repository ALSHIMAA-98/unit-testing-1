...
fdescribe("todo routes", () => {
    ...
    it("expect post(/todo) to add a todo", async () => {                  //to set key and value in the headers
        //adding todo
        let res = await req.post("/todo").send({ title: "eating launch" }).set({ authorization: token })
        expect(res.status).toBe(201)//201 for creating in DB
        todoInDB = res.body.data//to catch todo in DB
    })
    ...
    it("expect get(/todo/id) with valid auth to get message to login first", async () => {
        let res = await req.get("/todo/" + todoInDB._id).set({ authorization: token })// token from user created and signed up then took his token 
